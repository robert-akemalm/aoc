...
public class Day20 {
    ...
    private static void part1() throws Exception {
        List<Particle> particles = parse(DATA);
        List<Particle> originalOrder = new ArrayList<>(particles);
        particles.sort((p1, p2) -> {
            long sumA1 = p1.a.sum();
            long sumA2 = p2.a.sum();
            int cmp = Long.compare(sumA1, sumA2);
            if (cmp == 0) {
                long sumV1 = p1.v.sum();
                long sumV2 = p2.v.sum();
                cmp = Long.compare(sumV1, sumV2);
                if (cmp == 0) {
                    long sumP1 = p1.p.sum();
                    long sumP2 = p2.p.sum();
                    cmp = Long.compare(sumP1, sumP2);
                }
            }
            return cmp;
        });
        ...
        for (int i = 0; i < originalOrder.size(); i++) {
            if (originalOrder.get(i) == particles.get(0)) {
                System.out.println(i); //457
            }
        }
    }
    ...
    private static void part2() throws Exception {
        ...
        for (int tick = 0; tick < Integer.MAX_VALUE; tick++) {
            boolean[] toRemove = new boolean[particles.size()];
            ...
            if (tick % 1_000 == 0) {
                System.out.println(tick + ": " + particles.size());
            }
    ...
    static class Particle {
        ...
        public boolean equals(Object other) {
            ...
            return p.x == o.p.x && p.y == o.p.y && p.z == o.p.z;
        }
    ...
    static List<Particle> parse(String s) {
        List<Particle> particles = new ArrayList<>();
        for (String line : s.split("\n")) {
            String[] parts = line.split(", ");
            XYZ p = XYZ.parse(parts[0].substring(parts[0].indexOf("<") + 1, parts[0].indexOf(">")));
            XYZ v = XYZ.parse(parts[1].substring(parts[1].indexOf("<") + 1, parts[1].indexOf(">")));
            XYZ a = XYZ.parse(parts[2].substring(parts[2].indexOf("<") + 1, parts[2].indexOf(">")));

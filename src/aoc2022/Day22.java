package aoc2022;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.regex.Matcher;
import java.util.regex.Pattern;

public class Day22 {
    public static void main(String[] args) {
        Util.time(() -> a(TEST_INPUT));
        Util.time(() -> a(INPUT));
//        Util.time(() -> b(TEST_INPUT));
        Util.time(() -> b(INPUT));
    }

    private static void a(String input) {
        String[] parts = input.split("\n\n");
        Board board = Board.parse(parts[0]);
        Path path = Path.parse(parts[1]);
        Explorer explorer = board.start();
        for (Action action : path.actions) {
            explorer.perform(action);
        }
        long password = (explorer.row + 1) * 1000L + (explorer.col + 1) * 4L + explorer.direction.ordinal();
        System.out.println(password);
    }

    private static void b(String input) {
        String[] parts = input.split("\n\n");
        Board board = Board.parse(parts[0]);
        Path path = Path.parse(parts[1]);
        Explorer explorer = board.start();
        for (Action action : path.actions) {
            if (action instanceof Turn) {
                explorer.perform(action);
            } else if (action instanceof Move move){
                for (int i = 0; i < move.steps; i++) {
                    switch (explorer.direction) {
                    case UP -> explorer.upCube();
                    case RIGHT -> explorer.rightCube();
                    case DOWN -> explorer.downCube();
                    case LEFT -> explorer.leftCube();
                    }
                }
            }
        }
        long password = (explorer.row + 1) * 1000L + (explorer.col + 1) * 4L + explorer.direction.ordinal();
        System.out.println(password);
    }

    private static class Explorer {
        int row;
        int col;
        Direction direction = Direction.RIGHT;
        Board board;

        public Explorer(int row, int col, Board board) {
            this.row = row;
            this.col = col;
            this.board = board;
        }

        public void perform(Action action) {
            if (action instanceof Turn turn) {
                direction = direction.turn(turn);
            } else if (action instanceof Move move) {
                switch (direction) {
                case UP -> {
                    for (int i = 0; i < move.steps; i++) {
                        int newRow = row == 0 ? board.chars.length - 1 : row - 1;
                        while (board.chars[newRow][col] == ' ') {
                            newRow = newRow == 0 ? board.chars.length - 1 : newRow - 1;
                        }
                        if (board.chars[newRow][col] == '#') {
                            break;
                        } else {
                            row = newRow;
                        }
                    }
                }
                case RIGHT -> {
                    char[] boardRow = board.chars[row];
                    for (int i = 0; i < move.steps; i++) {
                        int newCol = col == boardRow.length - 1 ? 0 : col + 1;
                        while (boardRow[newCol] == ' ') {
                            newCol = newCol == boardRow.length - 1 ? 0 : newCol + 1;
                        }
                        if (boardRow[newCol] == '#') {
                            break;
                        } else {
                            col = newCol;
                        }
                    }
                }
                case DOWN -> {
                    for (int i = 0; i < move.steps; i++) {
                        int newRow = row == board.chars.length - 1 ? 0 : row + 1;
                        while (board.chars[newRow][col] == ' ') {
                            newRow = newRow == board.chars.length - 1 ? 0 : newRow + 1;
                        }
                        if (board.chars[newRow][col] == '#') {
                            break;
                        } else {
                            row = newRow;
                        }
                    }
                }
                case LEFT -> {
                    char[] boardRow = board.chars[row];
                    for (int i = 0; i < move.steps; i++) {
                        int newCol = col == 0 ? boardRow.length - 1 : col - 1;
                        while (boardRow[newCol] == ' ') {
                            newCol = newCol == 0 ? boardRow.length - 1 : newCol - 1;
                        }
                        if (boardRow[newCol] == '#') {
                            break;
                        } else {
                            col = newCol;
                        }
                    }
                }
                }
            }
        }

        private void rightCube() {
            int col;
            int row;
            Direction direction;

            if (this.col == 149 && this.row >= 0 && this.row < 50) {
                col = 99;
                row = 149 - this.row;
                direction = Direction.LEFT;
            } else if (this.col == 99 && this.row >= 50 && this.row < 100) {
                col = 50 + this.row;
                row = 49;
                direction = Direction.UP;
            } else if (this.col == 99 && this.row >= 100 && this.row < 150) {
                col = 149;
                row = 149 - this.row;
                direction = Direction.LEFT;
            } else if (this.col == 49 && this.row >= 150 && this.row < 200) {
                col = this.row - 100;
                row = 149;
                direction = Direction.UP;
            } else {
                col = this.col + 1;
                row = this.row;
                direction = Direction.RIGHT;
            }

            if (board.chars[row][col] == '.') {
                this.col = col;
                this.row = row;
                this.direction = direction;
            }
        }

        private void leftCube() {
            int col;
            int row;
            Direction direction;
            if (this.col == 50 && this.row >= 0 && this.row < 50) {
                col = 0;
                row = 149 - this.row;
                direction = Direction.RIGHT;
            } else if (this.col == 50 && this.row >= 50 && this.row < 100) {
                col = this.row - 50;
                row = 100;
                direction = Direction.DOWN;
            } else if (this.col == 0 && this.row >= 100 && this.row < 150) {
                col = 50;
                row = 149 - this.row;
                direction = Direction.RIGHT;
            } else if (this.col == 0 && this.row >= 150 && this.row < 200) {
                col = this.row - 100;
                row = 0;
                direction = Direction.DOWN;
            } else {
                col = this.col - 1;
                row = this.row;
                direction = Direction.LEFT;
            }

            if (board.chars[row][col] == '.') {
                this.row = row;
                this.col = col;
                this.direction = direction;
            }
        }

        private void downCube() {
            int col;
            int row;
            Direction direction;

            if (this.row == 49 && this.col >= 100 && this.col < 150) {
                col = 99;
                row = this.col - 50;
                direction = Direction.LEFT;
            } else if (this.row == 149 && this.col >= 50 && this.col < 100) {
                col = 49;
                row = this.col + 100;
                direction = Direction.LEFT;
            } else if (this.row == 199 && this.col >= 0 && this.col < 50) {
                col = this.col + 100;
                row = 0;
                direction = Direction.DOWN;
            } else {
                col = this.col;
                row = this.row + 1;
                direction = Direction.DOWN;
            }

            if (board.chars[row][col] == '.') {
                this.col = col;
                this.row = row;
                this.direction = direction;
            }
        }

        private void upCube() {
            int col;
            int row;
            Direction direction;

            if (this.row == 100 && this.col >= 0 && this.col < 50) {
                col = 50;
                row = 50 + this.col;
                direction = Direction.RIGHT;
            } else if (this.row == 0 && this.col >= 50 && this.col < 100) {
                col = 0;
                row = this.col + 100;
                direction = Direction.RIGHT;
            } else if (this.row == 0 && this.col >= 100 && this.col < 150) {
                col = this.col - 100;
                row = 199;
                direction = Direction.UP;
            } else {
                col = this.col;
                row = this.row - 1;
                direction = Direction.UP;
            }

            if (board.chars[row][col] == '.') {
                this.col = col;
                this.row = row;
                this.direction = direction;
            }
        }
    }

    private record Board(char[][] chars) {
        public static Board parse(String input) {
            String[] rows = input.split("\n");
            char[][] chars = new char[rows.length][];
            int maxCol = Arrays.stream(rows).mapToInt(String::length).max().orElse(0);
            for (int i = 0; i < rows.length; i++) {
                String row = rows[i];
                chars[i] = new char[maxCol];
                for (int col = 0; col < maxCol; col++) {
                    chars[i][col] = col < row.length() ? row.charAt(col) : ' ';
                }
            }
            return new Board(chars);
        }

        Explorer start() {
            for (int i = 0; i < chars[0].length; i++) {
                if (chars[0][i] == '.') {
                    return new Explorer(0, i, this);
                }
            }
            throw new IllegalStateException();
        }
    }

    private record Path(Action[] actions) {
        static Path parse(String input) {
            Pattern p = Pattern.compile("\\d+|\\D");
            Matcher m = p.matcher(input);
            List<Action> actions = new ArrayList<>();
            while (m.find()) {
                String group = m.group();
                if (group.charAt(0) == 'L') {
                    actions.add(Turn.L);
                } else if (group.charAt(0) == 'R') {
                    actions.add(Turn.R);
                } else {
                    actions.add(new Move(Integer.parseInt(group)));
                }
            }
            return new Path(actions.toArray(Action[]::new));
        }
    }

    private interface Action {}

    private enum Turn implements Action {R, L}

    private record Move(int steps) implements Action {}

    private enum Direction {
        RIGHT, DOWN, LEFT, UP;
        static final Direction[] ALL = Direction.values();

        Direction turn(Turn turn) {
            return turn == Turn.R ? ALL[(ordinal() + 1) % ALL.length] : ALL[(ALL.length + ordinal() - 1) % ALL.length];
        }
    }

    private static final String TEST_INPUT = """
                    ...#
                    .#..
                    #...
                    ....
            ...#.......#
            ........#...
            ..#....#....
            ..........#.
                    ...#....
                    .....#..
                    .#......
                    ......#.
                        
            10R5L5R10L4R5L5""";

    private static final String INPUT = """
                                                              .....#.............#.....#......##.......##...............#..#....................##................
                                                              ....#.#.........#.#..#.##....#...#.#.......#.#.#.#..........................................#......#
                                                              ............#..#..............#.##......#........................................#..#...#....#......
                                                              ............#.......#..........................#.#.................................#.........#......
                                                              ..#.....#.......#......#.......#.....................#.....#...##...#.......##....................#.
                                                              .........................#....#....#.......#..........#...............#...............#.............
                                                              .......#...............#...#...............#.......##.....................................#.#.......
                                                              ..............#.....................#..#............#.........#.........#.#...........#...........#.
                                                              #............#..........#.#....#............#.......#...............................#...............
                                                              ...................#..................#.............#........#........#...#.#.......#...........#...
                                                              .....##.......................................#.............#....#....#.....#.......#.......#.......
                                                              .............#..................#...............................##............#............#........
                                                              ......#..........#...#............#.............#........#........#.....#......#..............#..#..
                                                              ..................#..............#.#...#.....#..........................#............#.........#....
                                                              #..#........#......#...............#........#....#.................#.....#......#................#..
                                                              ........#.............#.....#........................#.##........#..#...............#........#......
                                                              .........###...........#...#........#.#..................##.......#...#..#..........................
                                                              .....................#..........#...........#..#........#..#.#........#.#..#............#...........
                                                              ..#..........................................#.#..#......................................#..........
                                                              .....#..#..#....................................##...................##.#.........#...#.......#.....
                                                              .....#....#.#....................##..#..........................#...................#.........#..#..
                                                              ...#........#..#.........#....................#..........#.........##...........#.........###.#.....
                                                              ...#...##......#..........#......................##.....................#............#.#............
                                                              .......................#....................#............#.#.....#...................#..............
                                                              ....#.....#..........#.................#...........#............#...........#........#....#....##...
                                                              ....#.#..............................#..........#.....#.#.........#...#.#.....#.....................
                                                              ..#..#..........#.#...##.#..............................#.#...............##.....................#.#
                                                              .##............#..#.......#............#.#..#............##..#...........#...#...#...#..............
                                                              .....#...............................................................#...#...................#......
                                                              ........#..#.........#..................#.............#...#..................#...............#......
                                                              ...........##...........#.....................#..#.....#.........##.................................
                                                              .#.........#...#.....##.#...#.....................#...##..#......#..........#.............#..##....#
                                                              ....................#.......#.....................#..............#....#.......#..................#..
                                                              ...#.......#..................#....#........#........#........................#......#..........#...
                                                              ..................#...............#....##........#...#.........................##.....#.............
                                                              ..............#..............##.......#..............#...#.........................................#
                                                              #.............................##................#...#..............#............#...#....#..........
                                                              ...............................#............#.................#..#.#.......##.....#.................
                                                              .....#.........................#..............#...............#.........#...........................
                                                              ......#..........#..#.................#..........#.........#........................##...#..#.......
                                                              ............#.#........#....#.#.......#......................................#.............#........
                                                              ....................#.......................#..............................#.......#....#........#..
                                                              .......#.................#...#......#...........#..#.#.................##...........#.....#...#.#...
                                                              ...#...................#........#..............#.#.....#..........#.............................#...
                                                              ......#.............#..#...#.....#...##.....................#.......#..........#.....##.............
                                                              #.#..............##..#..........................#....##...#......................................##.
                                                              ...##...............#.#..#................................#......#..................#.....#.#.......
                                                              ........#......#........#..........#.................#..#..#....#.........#...#..#.........#........
                                                              ##.........#...............#................................................#....................#.#
                                                              ..#..............#...................#..........................................#...................
                                                              ....#.#.##.....#......##.#.......##.#........#....
                                                              #................##.....##.........#..............
                                                              ......#.........#....#...#....#......##...........
                                                              .#..................#..#..##....#..#..............
                                                              .#............#........#......................#...
                                                              ............##......#...........#......#....#.....
                                                              ...##..............#.....#........##......#...##..
                                                              ...##.....#..#...........#....#........#..........
                                                              .............................................#...#
                                                              ...#.......#.......#..............................
                                                              #.................#.##.....#....#.......#.##......
                                                              ....#.....#..#............#..........##.#.........
                                                              ......#.#.#.......#..#..........##..#.#...........
                                                              .......#..........................................
                                                              ..#.......#........#..#.........#.................
                                                              #....#..........#......................#....#...#.
                                                              ..........#......##..#...............##.##........
                                                              .#................................................
                                                              ..#.###...................#...#.......#...........
                                                              ..#.....#.#......#......#..#...#.............#....
                                                              ..#.........#..............#....#..........#..#...
                                                              ...........#.#.....#...#....#...............#.....
                                                              ..#.....................................#.........
                                                              ................#.#........###.....#.....#.#....#.
                                                              .........................##.......................
                                                              .#.....#...#.#..........#...#..#..#...............
                                                              .............#........#..#...#...#.........#....##
                                                              ..#...##...##.....#......#.##.....................
                                                              .#.#.....#...............#........................
                                                              ......#.........#.#..........#....#.#.............
                                                              .............................#.......##...........
                                                              #...........#.....................................
                                                              ......#....#..###.#..#.....................#......
                                                              ............#...#..........#......................
                                                              #...#.......#........................#..#.........
                                                              ...#............#......#............#........#....
                                                              ......#.....#..#...#...#........##..#..#..........
                                                              .................#.................#..............
                                                              ........#......................#.........#..#.....
                                                              ..................#................#..............
                                                              ..#...#.......#...................................
                                                              #..............................#..#..##...........
                                                              ..#.......#.#......#...#..........#...............
                                                              ...........#........#..#.....#...#.....#..........
                                                              #..........#............#.....#.................#.
                                                              ....#....#.#...............##....#................
                                                              #.......#....#..............#.#...................
                                                              .........................#......#.............#..#
                                                              ....................#.....#......................#
                                                              ..#........#.........#.......#..................#.
            ...........#..#...#..#........#....#...............#........#..#.#......#............#..........#..#
            ........#......#........#...............#....##.................#.#..........#..........##....#.....
            ..........................#...#..............#.....#.#......................#..........#..........#.
            #....##..................#...#.....#............#..............#....................................
            #........................................................#..........................................
            ..............................#.................#..........#...#.#.....................##.........#.
            .....#.#......#.#........###..#.........#...#...........#............#.............#....#........#..
            .....#..........#.#..............#......#.................#.......#....#............................
            ...##....##...##...........#......#...#.....#.#......#........................#.....................
            #...#......#..................................#...#......................#..#..............#..#....#
            .......#......#..#.............#..#...........................................#......#.#....#.#.....
            ##.#..#.#..................#................#.#.......................................#...#.........
            .....#...............#.......................#....#............................................#....
            ...........#...........................#.......................#....#.....#.....#..................#
            ............#.......#..........#......................................#.#.................#.....#.##
            ..#........#...#.......#..#...#..................#...#.................#.#....#...............##...#
            ................#...................#.#.......#.......#............#.....................#..........
            .........#......................#.........#........#........#....#.#....###............##.#......#..
            .......................#.........................#...............................#............#.....
            #.......#...#..#......................#..............................#...........#.........#..#.....
            .##....#............#......#............#.....#...........#...........#......#....#.................
            ...............#.....................................#...............#......#.........#......#......
            ...#......................#..........#.......##......##...#................................#.#......
            ...............#..#...................#................#.#.......#.......#..........#.#.............
            .#...#...#...##.....#.......##....#......#......#.#.....#...............................#....#...#..
            ...............#..............#.#....#.#...................#..#.......#.............................
            ..........#..#.#..........#..........#....#.#..........###........#.......#...#.................#...
            #........#...........###..........##..................#..#....#.......#.#.........................#.
            .#..#.......##...............#..........................#.................#...............#.#......#
            .#...#................#...............................#..........#..#....#..#.......................
            ..............#.......#.............................#.............................##.........#......
            ......#...#..........#.........#......#...........#.........................#....#.............#...#
            .........#..#.#....#.............#...............................#.....##..............#...#...#....
            ..................#.......#.......#.............................#.............................#....#
            .###......#......#.#.#.....#..............#.#.....##..........#..............#......................
            ......#....#........#.......#.#.................#........##..............#.#....#...........###.....
            ..#.##...........................#......#.............................#.....................#.......
            .....................#......#...#.............................#.#..#.......#.......#...#.#...#......
            ....#....................##...............#..##......#......#......#.......##..........#...........#
            ..................#.........#...........##.............#..........#.......#..............#..........
            ..#.............#...................#.............#.##.#......#......#................#....#........
            ...........#..#............................#.#.#.........#..#........##....#........................
            .#.........................#.#.....................#......#...............#..........#..............
            ....................#.........#........................#........#........#...........#....#.......#.
            ..#...........................#.#.......#....#.........#...#....#.#.......#..........#..#...........
            .#........#.................#...#........#.........#......................#.........#.#.............
            .....#....#....#...............#........##..................#..#...........................#.......#
            ....................#..............#..........................................#.......#........#....
            ....#...#.#...............#..#..#......##...............##.............#.......#....................
            .#.#....#.#.......#...#......#......#....#.#..............##.........#.....#...#........#...........
            .....#...#..#........#.........#..................
            #..#...............#...................#........#.
            ....................................#.........#...
            ..........#..#..........#..#.........#............
            ..................#.........#...................#.
            #.........#...........#.........#...............##
            ............#......#.....#............#...........
            ...................................#..............
            .......#.........#............#...................
            .....#.......................#.................#..
            ..............#...#.......#..........###..##......
            .........##.......#...........#.......#..........#
            ..#.#.....#....#........................#....#....
            .......#..#......#...........#.#..................
            ......#.....#....#......#.................#.##....
            ...............#.........................#........
            ...#....#.................##.#....#..#............
            .....#..........#..#..............#.....#.........
            ...#..................#............#....#.........
            ...........##................................#....
            ..............#...................................
            ..........#...#..#.....#...................#......
            #..................#............#.......#.........
            ......#.....#..........#..........................
            .........#....#...........................#......#
            ..#......#.....#..................#...............
            ...............#....#..#.................#..#.....
            .#.#....................#................#.......#
            ..#....#..............##..#.......................
            ...#....#.......#...........#..#......#...........
            ..#...............#....#.##.............#.........
            ................#..........#......................
            ..##..........#......#......#.......#.#...#...#...
            ..................................#...#..........#
            .....................#....................#.......
            ...#.............#...#.......#....#........##.#...
            ............#....#.#.#.............#.............#
            .................................##...#.....#.....
            ...............#.#.#.#..........................#.
            .........#..................#....#...#.....#.#...#
            .............#....#.#...................#.........
            ..#.#.............#.............#........#........
            ...#......#............#........##......#.........
            .....#...#.#...........#..##......................
            ....#.........#.....#................#............
            ....#....#........#................#.#........#...
            #..#...................#............#.............
            ....#.........#..#..#.....#.............#.........
            .....#..........................#.............#...
            ....................#.........#.#................#
                        
            """;
}

// > 100.000

// 122.153
// < 125.000
// < 156.054
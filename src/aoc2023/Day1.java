...
public class Day1 {
    ...
    private static void a(String input) {
        ToIntFunction<String> first = s -> s.chars().filter(Character::isDigit).findFirst().orElseThrow() - '0';
        ToIntFunction<String> last = s -> s.chars().filter(Character::isDigit).reduce((a, b) -> b).orElseThrow() - '0';
        ToIntFunction<String> extract = s -> first.applyAsInt(s) * 10 + last.applyAsInt(s);
        int sum = input.lines().mapToInt(extract).sum();
    ...
    private static void b(String input) {
        ...
        ToIntFunction<String> first = s -> numbers.entrySet().stream()
                                                  .filter(e -> s.contains(e.getKey()))
                                                  .sorted(comparingInt(e -> s.indexOf(e.getKey())))
                                                  .mapToInt(Entry::getValue)
                                                  .findFirst().orElseThrow();

        ToIntFunction<String> last = s -> numbers.entrySet().stream()
                                                 .filter(e -> s.contains(e.getKey()))
                                                 .sorted(comparingInt(e -> -s.lastIndexOf(e.getKey())))
                                                 .mapToInt(Entry::getValue)
                                                 .findFirst().orElseThrow();
    ...
    private static final String TEST_INPUT = "1abc2\n"
                                             + "pqr3stu8vwx\n"
                                             + "a1b2c3d4e5f\n"
                                             + "treb7uchet";

    private static final String TEST_INPUT_2 = "two1nine\n"
                                               + "eightwothree\n"
                                               + "abcone2threexyz\n"
                                               + "xtwone3four\n"
                                               + "4nineeightseven2\n"
    ...
    private static final String INPUT = "dssmtmrkonedbbhdhjbf9hq\n"

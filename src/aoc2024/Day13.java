...
public class Day13 {
    private static void a(Input input) {
        ...
        for (Machine machine : input.machines) {
            PriorityQueue<State> states = new PriorityQueue<>(Comparator.comparingLong(State::cost));
            states.add(new State(new Pos(0, 0), 0, 0, 0));
            Set<Pos> seen = new HashSet<>();
            long minCost = Long.MAX_VALUE;
            while (!states.isEmpty()) {
                State state = states.poll();
                if (state.stepsA > 100 || state.stepsB > 100
                    || state.pos.x() > machine.prize.x() || state.pos.y() > machine.prize.y()
                    || !seen.add(state.pos())) {
                    continue;
                }
                if (state.pos().equals(machine.prize())) {
                    minCost = state.cost();
                    break;
                }
                states.add(new State(state.pos().move(machine.a.x(), machine.a.y()), state.cost() + 3, state.stepsA + 1,
                        state.stepsB));
                states.add(new State(state.pos().move(machine.b.x(), machine.b.y()), state.cost() + 1, state.stepsA,
                        state.stepsB + 1));
            }
            total += minCost == Long.MAX_VALUE ? 0 : minCost;
        }
        System.out.println(total);
    }

    private static void b(Input input) {
        // stepsA * m.a.x + stepsB * m.b.x = prize.x;    ⎡m.a.x, m.b.x⎤ ⎡stepsA⎤ = ⎡prize.x⎤
        // stepsA * m.a.y + stepsB * m.b.y = prize.y;    ⎣m.a.y, m.b.y⎦ ⎣stepsB⎦ = ⎣prize.y⎦

        long total = 0;
        for (Machine m : input.machines) {
            long prizeX = m.prize.x() + 10000000000000L;
            long prizeY = m.prize.y() + 10000000000000L;
            long stepsA = (prizeX * m.b.y - prizeY * m.b.x) / (m.a.x * m.b.y - m.a.y * m.b.x);
            long stepsB = (prizeX * m.a.y - prizeY * m.a.x) / (m.b.x * m.a.y - m.b.y * m.a.x);
            if (stepsA >= 0 && stepsB >= 0 &&
                stepsA * m.a.x + stepsB * m.b.x == prizeX &&
                stepsA * m.a.y + stepsB * m.b.y == prizeY) {
                total += stepsA * 3 + stepsB;
            }
        }
        System.out.println(total);
    }
    ...
    record Pos(long x, long y) {
        Pos move(long dx, long dy) {
            return new Pos(x + dx, y + dy);
        }
    }

    record State(Pos pos, long cost, int stepsA, int stepsB) {}
    ...
    record Input(List<Machine> machines) {
        static Input parse(String input) {
            return new Input(Arrays.stream(input.split("\n\n"))
                                   .map(m -> m.lines().map(Util::extractInts).map(a -> new Pos(a[0], a[1])).toList())
                                   .map(m -> new Machine(m.get(0), m.get(1), m.get(2)))
                                   .toList());
        }
    }

    private static final String TEST_INPUT = "Button A: X+94, Y+34\n"
                                             + "Button B: X+22, Y+67\n"
                                             + "Prize: X=8400, Y=5400\n"
                                             + "\n"
                                             + "Button A: X+26, Y+66\n"
    ...
    private static final String INPUT = "Button A: X+43, Y+21\n"
                                        + "Button B: X+16, Y+31\n"
                                        + "Prize: X=6735, Y=9135\n"
                                        + "\n"
                                        + "Button A: X+52, Y+55\n"

...
public class Day3 {
    ...
    private static void a(Input input) {
        long sum = input.parts.stream()
                              .filter(p -> input.symbols.stream().anyMatch(p::isConnectedTo))
                              .mapToInt(Part::value)
                              .sum();
    ...
    private static void b(Input input) {
        long sum = input.symbols.stream().filter(s -> s.c == '*')
                                .map(s -> input.parts.stream().filter(p -> p.isConnectedTo(s)).toList())
                                .filter(connections -> connections.size() == 2)
                                .mapToInt(connections -> connections.get(0).value * connections.get(1).value)
                                .sum();
        System.out.println(sum);
    }

    record Part(int row, int start, int end, int value) {
        boolean isConnectedTo(Symbol symbol) {
            return row - 1 <= symbol.row && symbol.row <= row + 1 && start - 1 <= symbol.col && symbol.col <= end;
        }
    }

    record Symbol(int row, int col, char c) {}

    record Input(Set<Part> parts, Set<Symbol> symbols) {
        static Input parse(String input) {
            ...
            for (int row = 0; row < rows.size(); row++) {
                ...
                StringBuilder currentNr = new StringBuilder();
                for (int col = 0; col < line.length(); col++) {
                    char c = line.charAt(col);
                    if (Character.isDigit(c)) {
                        currentNr.append(c);
                    } else {
                        if (!currentNr.isEmpty()) {
                            int start = col - currentNr.length();
                            parts.add(new Part(row, start, col, Integer.parseInt(currentNr.toString())));
                            currentNr.setLength(0);
                        }
                        if (c != '.') {
                            symbols.add(new Symbol(row, col, c));
                        }
                    }
                }
                if (!currentNr.isEmpty()) {
                    int start = line.length() - currentNr.length();
                    parts.add(new Part(row, start, line.length(), Integer.parseInt(currentNr.toString())));
                }
            }
            return new Input(parts, symbols);
        }
    }

    private static final String TEST_INPUT = "467..114..\n"
                                             + "...*......\n"
                                             + "..35..633.\n"
                                             + "......#...\n"
                                             + "617*......\n"

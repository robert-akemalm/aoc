...
public class Day16 {
    ...
    private static void b(String input) {
        Validator[] validators = input.lines().filter(s -> s.contains(" or ")).map(Validator::new).toArray(Validator[]::new);
        Predicate<int[]> valid = ticket -> {
            for (int val : ticket) {
                boolean ok = Arrays.stream(validators).anyMatch(v -> v.test(val));
                if (!ok) {
                    return false;
                }
            }
        ...
        Ticket myTicket = input.lines().filter(Day16::isTicketRow).findFirst().stream().map(Day16::ticket).findFirst().map(Ticket::new).get();
        Ticket[] otherTickets = input.lines().filter(Day16::isTicketRow).skip(1).map(Day16::ticket).filter(valid).map(Ticket::new).toArray(Ticket[]::new);
        Field[] fields = new Field[validators.length];
        for (int i = 0; i < validators.length; i++) {
            fields[i] = new Field(i, validators[i]);
        }
        long[] all = {(long)(Math.pow(2, fields.length) - 1)};
        BitSet[] possibleFields = IntStream.range(0, fields.length).mapToObj(i -> BitSet.valueOf(all)).toArray(BitSet[]::new);
        for (int fieldIx = 0; fieldIx < possibleFields.length; fieldIx++) {
            BitSet p = possibleFields[fieldIx];
            for (int i = p.nextSetBit(0); i != -1; i = p.nextSetBit(i + 1)) {
                for (Ticket ticket : otherTickets) {
                    if (!fields[i].validator.test(ticket.values[fieldIx])) {
                        p.clear(i);
                    }
                }
            }
        ...
        int[] fieldOrder = reduce(possibleFields);
        long prod = 1;
        for (Field field : fields) {
            if (field.name.startsWith("departure")) {
                for (int ticketIx = 0; ticketIx < fieldOrder.length; ticketIx++) {
                    if (fieldOrder[ticketIx] == field.ix) {
                        prod *= myTicket.values[ticketIx];
                    }
                }
            }
        }
    ...
    private static int[] reduce(BitSet[] possibleFields) {
        int[] fixedLastTime = new int[0];
        while (true) {
            int[] fixed = IntStream.range(0, possibleFields.length)
                                   .filter(val -> Arrays.stream(possibleFields).filter(bs -> bs.get(val)).count() == 1)
                                   .toArray();
            for (int val : fixed) {
                for (BitSet bs : possibleFields) {
                    if (bs.get(val)) {
                        bs.clear();
                        bs.set(val);
                    }
            ...
            if (Arrays.equals(fixed, fixedLastTime)) {
                break;
            }
            fixedLastTime = fixed;
        }
        if (Arrays.stream(possibleFields).anyMatch(bs -> bs.cardinality() != 1)) {
            System.out.println("Multiple interpretations");
        }
        return Arrays.stream(possibleFields).mapToInt(bs -> bs.nextSetBit(0)).toArray();
    }
    ...
    private static class Field {
        ...
        private Field(int ix, Validator validator) {
            this.ix = ix;
            this.name = validator.field;
    ...
    private static void a(String input) {
        Validator[] validators = input.lines().filter(s -> s.contains(" or ")).map(Validator::new).toArray(Validator[]::new);
        int[][] otherTickets = input.lines().filter(Day16::isTicketRow).skip(1).map(Day16::ticket).toArray(int[][]::new);
        long errorRate = 0;
        for (int[] t : otherTickets) {
            for (int val : t) {
                boolean valid = Arrays.stream(validators).anyMatch(v -> v.test(val));
                if (!valid) {
                    errorRate += val;
                }
            }
    ...
    private static boolean isTicketRow(String row) {
        return !row.isEmpty() && Character.isDigit(row.charAt(0));
    }

    private static int[] ticket(String row) {
        return Arrays.stream(row.split(",")).mapToInt(Integer::parseInt).toArray();
    }

    private static class Validator implements IntPredicate {
        final String field;
        final BitSet validValues;

        private Validator(String info) {
            this.field = info.substring(0, info.indexOf(":"));
            this.validValues = new BitSet();
            Arrays.stream(info.split(" ")).filter(p -> Character.isDigit(p.charAt(0))).forEach(s -> {
    ...
    private static final String TEST_INPUT = "class: 0-1 or 4-19\n"
                                             + "row: 0-5 or 8-19\n"
                                             + "seat: 0-13 or 16-19\n"
                                             + "\n"
                                             + "your ticket:\n"
    ...
    private static final String INPUT = "departure location: 48-793 or 800-971\n"

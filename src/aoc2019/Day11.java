...
public class Day11 {
    ...
    private static void b(String input) {
        ...
        Map<Pos, Integer> colored = new HashMap<>();
        colored.put(robot.pos, 1);
        IO read = new IO();
        IO write = new IO();
        IntComputer intComputer = new IntComputer(read, write, input);
        while (intComputer.reader.ix != Integer.MAX_VALUE) {
            read.write(colored.getOrDefault(robot.pos, 0));
            intComputer.run();
            int color = write.values.pop().intValue();
            int turn = write.values.pop().intValue();
            colored.put(robot.pos, color);
            if (turn == 0) {
                robot.turnLeft();
            } else {
                robot.turnRight();
        ...
        Set<Pos> white = colored.entrySet().stream().filter(e -> e.getValue() == 1).map(Entry::getKey).collect(Collectors.toSet());
        Util.print(white, '\u2588', ' ');
    }

...
public class Day4 {
    ...
    private static void b() {
        Map<String, Guard> guards = parseGuards();
        int maxId = 0;
        int maxSleepIx = 0;
        int maxSleep = 0;
        for (Guard g : guards.values()) {
            for (int i = 0; i < g.sleep.length; i++) {
                if (g.sleep[i] > maxSleep) {
                    maxSleep = g.sleep[i];
                    maxId = g.id;
                    maxSleepIx = i;
                }
        ...
        System.out.println(maxId * maxSleepIx);
    }
    ...
    private static void a() {
        ...
        int maxSleep = 0;
        for (Guard g : guards.values()) {
            int sleep = IntStream.of(g.sleep).sum();
            if (sleep > maxSleep) {
                maxSleepGuard = g;
                maxSleep = sleep;
            }
        ...
        int maxIx = 0;
        int[] sleep = maxSleepGuard.sleep;
        for (int i = 1; i < sleep.length; i++) {
            if (sleep[i] > sleep[maxIx]) {
                maxIx = i;
            }
        }
        System.out.println(maxSleepGuard.id * maxIx);
    }

    private static Map<String, Guard> parseGuards() {
        ...
        for (String line : lines) {
            int minute = Integer.parseInt(line.substring(line.indexOf(':') + 1, line.indexOf(']')));
            if (line.contains("Guard")) {
                int ix = line.indexOf('#') + 1;
                String id = line.substring(ix, line.indexOf(' ', ix));
                guard = guards.computeIfAbsent(id, __ -> new Guard(id));
    ...
    private static final String INPUT = "[1518-09-16 23:57] Guard #1889 begins shift\n"

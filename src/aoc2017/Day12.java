...
public class Day12 {
    ...
    private static void part1() throws Exception {
        ...
        for (String line : lines) {
            ...
            String[] connections = line.substring(line.indexOf(">") + 2).split(", ");
            nodes.put(node, Arrays.asList(connections));
        }

        Set<String> explored = new HashSet<>();
        Set<String> toExplore = new HashSet<>(nodes.get("0"));
        explored.add("0");

        while (toExplore.size() > 0) {
            Set<String> newToExplore = new HashSet<>();
            for (String node : toExplore) {
                if (explored.add(node)) {
                    newToExplore.addAll(nodes.get(node));
                }
            }
    ...
    private static void part2() throws Exception {
        ...
        int count = 0;
        for (String program : nodes.keySet()) {
            if (explored.add(program)) {
                toExplore.addAll(nodes.get(program));
                while (toExplore.size() > 0) {
                    Set<String> newToExplore = new HashSet<>();
                    for (String node : toExplore) {
                        if (explored.add(node)) {
    ...
    private static final String DATA2 = "0 <-> 2\n"
                                        + "1 <-> 1\n"
                                        + "2 <-> 0, 3, 4\n"
                                        + "3 <-> 2, 4\n"
                                        + "4 <-> 2, 3, 6\n"
    ...
    private static final String DATA = "0 <-> 584, 830\n"
                                       + "1 <-> 415, 531\n"
                                       + "2 <-> 514, 1419\n"
                                       + "3 <-> 3\n"
                                       + "4 <-> 1043\n"

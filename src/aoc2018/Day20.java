...
public class Day20 {
    ...
    private static int nodeId(int x, int y) {
    ...
    private static void a(String input) {
        ...
        for (int i = 1; i < input.length() - 1; i++) {
            ...
            case '|':
                ...
                break;
            case 'N':
                id = nodeId(current.x, current.y-1);
                p = points.get(id);
                if (p == null || p.distance > current.distance + 1) {
                    p = new Point(current.x, current.y-1, current.distance + 1);
                    points.put(id, p);
                }
                current = p;
                break;
            case 'S':
                ...
                break;
            case 'W':
                id = nodeId(current.x-1, current.y);
                p = points.get(id);
                if (p == null || p.distance > current.distance + 1) {
                    p = new Point(current.x-1, current.y, current.distance + 1);
                    points.put(id, p);
                }
                current = p;
                break;
        ...
        for (Point p : points.values()) {
            ...
            if (p.distance >= 1000) { // 8526 to high
                moreThan1000++;
            }
        }
    ...
    private static class Stack {
        private Point[] points = new Point[50_000];
        private int ix = 0;
    ...
    private static final String TEST_INPUT = "^ESSWWN(E|NNENN(EESS(WNSE|)SSS|WWWSSSSE(SW|NNNE)))$"; // 23
    private static final String TEST_INPUT_2 = "^WSSEESWWWNW(S|NENNEEEENN(ESSSSW(NWSW|SSEN)|WSWWN(E|WWS(E|SS))))$"; // 31

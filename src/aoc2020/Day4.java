...
//        String input = TEST_INPUT;
    ...
    enum Field {
        ...
        hgt(v -> {
            int i = Integer.parseInt(v.substring(0, v.length() - 2));
            boolean cm = v.endsWith("cm");
            if (cm) {
                return i >= 150 && i <= 193;
            } else {
                return v.endsWith("in") && i >= 59 && i <= 76;
            }
        }),
        hcl(v -> {
            if (!v.startsWith("#")) {
        ...
        ecl(v -> {
            return Arrays.asList("amb",  "blu", "brn", "gry", "grn", "hzl", "oth").contains(v);
        }),
        pid(v -> {
            return v.length() == 9 && v.chars().allMatch(c -> '0' <= c && c <= '9');
        }),
        cid(v -> { return true;
        });
        final Predicate<String> validator;

        Field(Predicate<String> validator) {
    ...
    private static void b(String input) {
        ...
        int inValid = 0;
        boolean[] validator = new boolean[Field.values().length];
        for (String passport : passports) {
            Arrays.fill(validator, false);
            validator[Field.cid.ordinal()] = true;
            String[] parts = passport.replaceAll("\n", " ").split(" ");
            for (String part : parts) {
                String[] keyValue = part.split(":");
                Field field = Field.valueOf(keyValue[0].trim());
                validator[field.ordinal()] = field.valid(keyValue[1].trim());
            }
            for (boolean b : validator) {
                if (!b) {
                    inValid++;
                    break;
                }
        ...
        System.out.println(passports.length - inValid);
    }
    ...
    private static final String TEST_INPUT = "ecl:gry pid:860033327 eyr:2020 hcl:#fffffd\n"
    ...
    private static final String INPUT = "hgt:159cm\n"

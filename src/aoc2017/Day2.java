...
public class Day2 {
    ...
    private static void part1() throws IOException {
        try (BufferedReader br = new BufferedReader(new StringReader(DATA))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                int min = Stream.of(numbers).map(Integer::parseInt).min(Comparator.comparingInt(i -> i)).get();
                int max = Stream.of(numbers).map(Integer::parseInt).max(Comparator.comparingInt(i -> i)).get();
                sum += max - min;
            }
    ...
    private static void part2() throws IOException {
        try (BufferedReader br = new BufferedReader(new StringReader(DATA))) {
            ...
            String line;
            while ((line = br.readLine()) != null) {
                Set<Integer> numbers = Stream.of(line.split("\t")).map(Integer::parseInt).collect(Collectors.toSet());
                for (int n1 : numbers) {
                    for (int n2 : numbers) {
                        if (n1 != n2 && n1 % n2 == 0) {
                            sum += n1 / n2;
    ...
    private static final String DATA = "1236\t741\t557\t1029\t144\t101\t1968\t2159\t1399\t80\t1139\t1167\t1695\t82\t90\t2236\n"

...
import static aoc2023.Day10.Node.GROUND;
...
public class Day10 {
    private static void a(Input input) {
        Sketch sketch = Sketch.make(input.nodes);
        Explorer explorer = new Explorer(sketch);
        System.out.println(explorer.findMaxDistanceFromStart());
    }

    private static void b(Input input) {
        Sketch sketch = Sketch.make(input.nodes);
        Explorer explorer = new Explorer(sketch);
        System.out.println(explorer.findNestingTiles());
    }

    record Sketch(Node[][] nodes, Node start) {
        public static Sketch make(Node[][] nodes) {
            Node start = null;
            for (int r = 0; r < nodes.length; r++) {
                for (int c = 0; c < nodes[r].length; c++) {
                    Node node = nodes[r][c];
                    final Node north = r == 0 ? GROUND : nodes[r - 1][c];
                    final Node south = r == nodes.length - 1 ? GROUND : nodes[r + 1][c];
                    final Node east = c == nodes[r].length - 1 ? GROUND : nodes[r][c + 1];
                    final Node west = c == 0 ? GROUND : nodes[r][c - 1];
                    switch (node.ch) {
                    case '|' -> node.addConnections(north, south);
                    case '-' -> node.addConnections(west, east);
                    case 'L' -> node.addConnections(north, east);
                    case 'J' -> node.addConnections(north, west);
                    case '7' -> node.addConnections(south, west);
                    case 'F' -> node.addConnections(south, east);
                    case '.' -> node.addConnections();
                    case 'S' -> {
                        start = node;
                        if (north.ch == '|' || north.ch == '7' || north.ch == 'F') {
                            node.addConnections(north);
                        }
                        if (south.ch == '|' || south.ch == 'L' || south.ch == 'J') {
                            node.addConnections(south);
                        }
                        if (east.ch == '-' || east.ch == 'J' || east.ch == '7') {
                            node.addConnections(east);
                        }
                        if (west.ch == '-' || west.ch == 'L' || west.ch == 'F') {
                            node.addConnections(west);
                        }
                    }
                    default -> throw new IllegalStateException("Unexpected value: " + node.ch);
                    }
                }
            }
            return new Sketch(nodes, start);
        }
    }

    static class Explorer {
        record State(Node node, long steps) {}

        final Sketch sketch;
        final PriorityQueue<State> toExplore = new PriorityQueue<>(Comparator.comparingLong(o -> o.steps));

        public Explorer(Sketch sketch) {
            this.sketch = sketch;
        }

        long findMaxDistanceFromStart() {
            long[][] distance = Arrays.stream(sketch.nodes)
                                      .map(r -> LongStream.range(0, r.length).map(d -> Long.MAX_VALUE).toArray())
                                      .toArray(long[][]::new);
            toExplore.add(new State(sketch.start, 0));
            while (!toExplore.isEmpty()) {
                State state = toExplore.poll();
                Node node = state.node;
                if (distance[node.row][node.col] > state.steps) {
                    distance[node.row][node.col] = state.steps;
                    for (Node neighbour : node.connections) {
                        if (distance[neighbour.row][neighbour.col] > state.steps + 1) {
                            toExplore.add(new State(neighbour, state.steps + 1));
                        }
                    }
                }
            }
            return Arrays.stream(distance).flatMapToLong(Arrays::stream).filter(d -> d != Long.MAX_VALUE).max().orElseThrow();
        }

        long findNestingTiles() {
            Node current = sketch.start;
            Set<Node> loop = new HashSet<>();
            Set<Node> blocksNorth = new HashSet<>();
            if (sketch.start.connections.stream().anyMatch(n -> n.col < sketch.start.col)) {
                blocksNorth.add(sketch.start);
            }
            while (loop.add(current)) {
                if (current.ch == '7' || current.ch == 'J' || current.ch == '-') {
                    blocksNorth.add(current);
                }
            ...
            long count = 0;
            for (int row = 0; row < sketch.nodes.length; row++) {
                for (int col = 0; col < sketch.nodes[row].length; col++) {
                    Node node = sketch.nodes[row][col];
                    if (!loop.contains(node)) {
                        long nBlocks = IntStream.range(0, row)
                                                .filter(r -> blocksNorth.contains(sketch.nodes[r][node.col]))
                                                .count();
                        if ((nBlocks % 2 == 1)) {
                            count++;
                        }
    ...
    record Node(char ch, int row, int col, List<Node> connections) {
        static final Node GROUND = new Node('.', -1, -1, List.of());

        void addConnections(Node... nodes) {
            connections.addAll(List.of(nodes));
        }
        ...
        public boolean equals(Object o) {
            ...
            Node node = (Node) o;
            return ch == node.ch && row == node.row && col == node.col;
        }
    ...
    record Input(Node[][] nodes) {
        static Input parse(String input) {
            List<String> lines = input.lines().toList();
            Node[][] nodes = lines.stream().map(l -> new Node[l.length()]).toArray(Node[][]::new);
            for (int row = 0; row < lines.size(); row++) {
                String line = lines.get(row);
                for (int col = 0; col < line.length(); col++) {
                    char ch = line.charAt(col);
                    nodes[row][col] = new Node(ch, row, col, new ArrayList<>());
                }
            }
            return new Input(nodes);
        }

    }

    private static final String TEST_INPUT2 = "-L|F7\n"
                                              + "7S-7|\n"
                                              + "L|7||\n"
                                              + "-L-J|\n"
                                              + "L|-JF";

    private static final String TEST_INPUT = "FF7FSF7F7F7F7F7F---7\n"
                                             + "L|LJ||||||||||||F--J\n"
                                             + "FL-7LJLJ||||||LJL-77\n"
                                             + "F--JF--7||LJLJ7F7FJ-\n"
                                             + "L---JF-JLJ.||-FJLJJ7\n"

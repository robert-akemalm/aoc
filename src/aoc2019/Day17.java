...
public class Day17 {
    ...
    private static void b(String input) {
        ...
        for (int i = 0; i < main.length(); i++) {
            read.write(main.charAt(i));
        }
        read.write('\n');

        String a = "R,10,R,10,R,6,R,4";
        for (int i = 0; i < a.length(); i++) {
            read.write(a.charAt(i));
        }
        read.write('\n');

        String b = "R,10,R,10,L,4";
        for (int i = 0; i < b.length(); i++) {
            read.write(b.charAt(i));
        }
        read.write('\n');

        String c = "R,4,L,4,L,10,L,10";
        for (int i = 0; i < c.length(); i++) {
            read.write(c.charAt(i));
        }
        read.write('\n');
        read.write('n');
        read.write('\n');

        IntComputer intComputer = new IntComputer(read, write, '2'+input.substring(1));
        intComputer.run();
        Long last = write.values.getLast();
    ...
    private static void a(String input) {
        ...
        while(true) {
            ...
            if (ch == '\n') {
                ...
                map.put(pos, ch.intValue());
                if (ch.intValue() == '<') {
                    robot.currentPos = pos;
                    robot.currentDir = Dir.L;
                } else if (ch.intValue() == '>') {
                    robot.currentPos = pos;
                    robot.currentDir = Dir.R;
                } else if (ch.intValue() == 'v') {
                    robot.currentPos = pos;
                    robot.currentDir = Dir.D;
                } else if (ch.intValue() == '^') {
                    robot.currentPos = pos;
                    robot.currentDir = Dir.U;
                }
            }
        }
        ...
        for (Entry<Pos, Integer> e : map.entrySet()) {
            if (e.getValue() == '#') {
                Pos pos = e.getKey();
                if (map.getOrDefault(new Pos(pos.x, pos.y -1), (int) '.') == '#' &&
                    map.getOrDefault(new Pos(pos.x, pos.y +1), (int) '.') == '#' &&
                    map.getOrDefault(new Pos(pos.x-1, pos.y), (int) '.') == '#' &&
                    map.getOrDefault(new Pos(pos.x+1, pos.y), (int) '.') == '#') {
                    intersections+= pos.x * pos.y;
                }
            }
        }
        ...
        while (true) {
            Pos p = robot.currentPos;
            Pos next = null;
            switch (robot.currentDir) {
            case U:
                next = new Pos(robot.currentPos.x, robot.currentPos.y - 1);
                if (map.getOrDefault(next, (int) '.') == '#') {
                    robot.currentPos = next;
                    robot.steps++;
                } else {
                    // turn right
                    next = new Pos(robot.currentPos.x+1, robot.currentPos.y);
                    if (map.getOrDefault(next, (int) '.') == '#') {
                        robot.currentPos = next;
                        robot.currentDir = Dir.R;
                        System.out.print(","+robot.steps + ",R");
                        robot.steps = 1;
                    } else {
                        // turn left
                        next = new Pos(robot.currentPos.x - 1, robot.currentPos.y);
                        if (map.getOrDefault(next, (int) '.') == '#') {
                            robot.currentPos = next;
                            robot.currentDir = Dir.L;
                            System.out.print(","+robot.steps + ",L");
                            robot.steps = 1;
                        }
                    }
                }
            ...
            if (robot.currentPos == p) {
                break;
            }
            visited.put(next, true);
        }
        System.out.println();
        //R,10,R,10,R,6,R,4,R,10,R,10,L,4,R,10,R,10,R,6,R,4,R,4,L,4,L,10,L,10,R,10,R,10,R,6,R,4,R,10,R,10,L,4,R,4,L,4,L,10,L,10,R,10,R,10,L,4,R,4,L,4,L,10,L,10,R,10,R,10,L
    }
